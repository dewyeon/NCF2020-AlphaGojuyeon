...
# original: https://raw.githubusercontent.com/pytorch/contrib/master/torchcontrib/optim/swa.py
...
class WeightAverageBuffer(object):
    def __init__(self, params, update_interval, sync_interval=10):
        self.param_buffer = [torch.zeros_like(p.detach()).copy_(p) for p in params]
        self.update_interval = update_interval
        self.sync_interval = sync_interval * update_interval
        self._reset()
    ...
    def step(self, model):
        ...
        # print(C.notice(f'SWA: {self.steps} / {self.sync_interval}'))

        if self.steps % self.update_interval == 0:
            for buff, new in zip(self.param_buffer, new_params):
                decay = 1. / (self.n_models + 1.)
                diff = (new.detach() - buff)
                buff.add_(decay * diff)
            self.n_models += 1

        if self.steps % self.sync_interval == 0:
            for param, buff in zip(model.parameters(), self.param_buffer):
                param.detach().copy_(buff.detach())
            self._reset()
            print(C.notice('SWA: sync params'))

    def parameters(self):
        return [param for param in self.param_buffer]


class SWA(Optimizer):
    def __init__(self, optimizer, update_interval, swap_interval):
        ...
        """
        assert swap_interval > update_interval
    ...
    def swap_params(self):
        ...
        print(C.notice('SWA: swap params'))
        self._reset()
